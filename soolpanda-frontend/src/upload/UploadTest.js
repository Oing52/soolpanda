...
const UploadTest = ({ onDrop, files }) => {
  const [previewFiles, setPreviewFiles] = useState([]);
  const [images, setImages] = useState([]);

  const handleDrop = (acceptedFiles) => {
    console.log(previewFiles[0].preview);
    ...
    if (previewFiles.length + acceptedFiles.length > 6) {
      ...
      setImages(acceptedFiles.preview);
    }
  };
              ...
              <div className="file_list">
                {previewFiles.map((file, index) => (
                  <div key={index} className="file_item" onClick={handleFileClick}>
                    ...
                    <img src={XButton} className="delete-button" alt="1" onClick={(event) => handleDelete(event, file)}></img>
                  </div>
                ))}
              </div>
            </div>
